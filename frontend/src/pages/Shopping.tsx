...
export default function Shopping() {
  ...
  return (
    <div className="space-y-8 max-w-3xl mx-auto">
      <div className="no-print bg-surface rounded-2xl p-6 sm:p-8 border border-zinc-200/80 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold uppercase tracking-wider text-ink-muted">Einkaufsliste</span>
            <span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-zinc-100">{plan.store}</span>
          </div>
        ...
        <div className="flex items-center gap-2 no-print">
          ...
          <button type="button" onClick={whatsapp} className="px-4 py-2.5 rounded-xl border border-zinc-200 text-xs font-bold">
            In WhatsApp öffnen
          </button>
          <button type="button" onClick={() => window.print()} className="px-3.5 py-2.5 rounded-xl border border-zinc-200">
            <Printer className="w-3.5 h-3.5" />
          </button>
        </div>
      </div>

      <div className="print-a4" aria-hidden>
        {[0, 1].map((col) => {
          const aisles = plan.shopping_list.to_buy
          const mid = Math.ceil(aisles.length / 2) || 1
          const slice = col === 0 ? aisles.slice(0, mid) : aisles.slice(mid)
          const leftover = col === 1 ? plan.shopping_list.already_at_home : []
          return (
            <>
            {col === 1 && <div className="print-cut" />}
            <div key={col} className="print-a5">
              <p style={{ fontWeight: 800, margin: 0 }}>NutriMatch · {plan.store}</p>
              <p style={{ margin: '2px 0 8px', fontSize: '8pt' }}>{plan.deal_week} · {plan.total_cost.toFixed(2)} € · A5 {col + 1}/2 · Schnittkante mittig</p>
              {slice.map((aisle) => (
                <div key={aisle.aisle} style={{ marginBottom: 8 }}>
                  <p style={{ fontWeight: 700, borderBottom: '1px solid #ddd', margin: '0 0 4px' }}>{aisle.aisle}</p>
                  {aisle.items.map((item) => (
                    <p key={item.name} style={{ margin: '2px 0' }}>
                      ☐ {item.name} — {itemLine(item)} {item.is_deal ? '★' : ''} {item.cost.toFixed(2)}€
                    </p>
                  ))}
                </div>
              ))}
              {leftover.length > 0 && (
                <div>
                  <p style={{ fontWeight: 700 }}>Schon im Vorrat</p>
                  {leftover.map((item) => (
                    <p key={item.name} style={{ margin: '2px 0' }}>· {item.name}</p>
                  ))}
                </div>
              )}
            </div>
      ...
      <div className="screen-list space-y-8">
      {plan.shopping_list.to_buy.map((aisle) => (
        <div key={aisle.aisle} className="bg-surface rounded-2xl p-6 border border-zinc-200/80 space-y-3">
      ...
      {plan.shopping_list.already_at_home.length > 0 && (
        <div className="bg-surface rounded-2xl p-6 border border-zinc-200/80 space-y-3">
      ...
      <div className="bg-surface rounded-2xl p-6 border border-zinc-200/80 flex items-center justify-between">
        <div>
